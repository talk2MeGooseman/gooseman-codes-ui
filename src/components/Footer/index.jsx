...
const ArrowIcon = ({ color, visible }) => (
  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ marginLeft: 'auto', flexShrink: 0, opacity: visible ? 1 : 0.4, transition: 'opacity 0.2s' }}>
    <path d="M4 12L12 4m0 0H5m7 0v7" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);

const ContactCard = ({ href, icon, label, sublabel, p }) => {
  ...
  return (
    ...
    >
      ...
      <ArrowIcon color={p.accent} visible={hovered} />
    </a>
  );
};
...
const Footer = ({ p }) => (
  <footer id="contact" style={{ borderTop: `1px solid ${p.border}` }}>
    <div style={{ padding: '56px 48px 48px', maxWidth: 900, margin: '0 auto' }}>
      <div style={{
